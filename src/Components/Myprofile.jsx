...
function Myprofile() {
    ...
    useEffect(() => {
      fetch(`https://api.github.com/users/${param.user.login}`).then(res => res.json()).then(data => setData(data))
      fetch(`https://api.github.com/users/${param.user.login}/repos`).then(res => res.json()).then(data => setrepo(data))
    }, [param.id])
    console.log(repo);
    return (
      <div className='bg-black text-white py-24 h-fit w-full md:flex flex-col items-center'>
        <p className='text-5xl text-center text-cyan-600 font-semibold '>My GitHub Profile</p>
